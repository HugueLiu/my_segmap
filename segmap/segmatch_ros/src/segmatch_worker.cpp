...
// 处理本地点云：分割，匹配，回环检测
// 加载当前地图，机器人的最新位姿，指定要处理的机器人
// loop_closure用来存储检测到的回环
// 返回是否检测到回环
...
void SegMatchWorker::publishTargetRepresentation() const {
  ...
  // 将点云整体向下移动一定距离，避免显示时与局部地图重叠
  translateCloud(Translation(0.0, 0.0, -params_.distance_to_lower_target_cloud_for_viz_m), &target_representation);
  // 格式转换
  sensor_msgs::PointCloud2 target_representation_as_message;
  convert_to_point_cloud_2_msg(target_representation, params_.world_frame, &target_representation_as_message);
